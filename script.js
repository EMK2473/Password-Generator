...
function generate_password() 
{
 ...
 if (quantity < 8 || quantity > 128){
  ...
  for (let i = 1; i <= quantity; i++) { // loop to create password
    ...
    let character_selection = Math.floor(Math.random() * type[type_selection].length); //gives me a random index from randomly selected string (which loops depending on quantity)
